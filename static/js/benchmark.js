class BenchmarkManager {
    ...
    async startBenchmark(modelName) {
        if (this.activeBenchmarks.has(modelName)) {
            this.showError(`Benchmark already running for model ${modelName}`, document.getElementById('benchmark-results-list'));
            return;
        }
        ...
        try {
            ...
            this.showError(`Failed to start benchmark for ${modelName}: ${error.message}`, document.getElementById('benchmark-results-list'));
        }
    }
    ...
    async refreshBenchmarks() {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch benchmark results: ${response.statusText}`);
            }
            const data = await response.json();
            this.updateBenchmarksList(data.results || [], data.active_benchmarks || []);
        } catch (error) {
            console.error('Failed to fetch benchmark results:', error);
            this.showError(`Failed to fetch benchmark results: ${error.message}`, document.getElementById('benchmark-results-list'));
        }
    }

    calculateAverageMetrics(metrics) {
        if (!metrics || metrics.length === 0) return { cpu: 0, memory: 0 };
        const sum = metrics.reduce((acc, metric) => ({
            cpu: acc.cpu + metric.cpu_percent,
            memory: acc.memory + metric.memory_percent
        }), { cpu: 0, memory: 0 });
        return {
    ...
    updateBenchmarksList(results, activeBenchmarks) {
        ...
        if (results.length === 0 && activeBenchmarks.length === 0) {
            const row = document.createElement('tr');
            row.innerHTML = '<td colspan="6" class="text-center text-muted">No benchmark results available</td>';
        ...
        results.forEach(result => {
            ...
            row.innerHTML = `
                <td>${result.model}</td>
                <td>${result.elapsed_time.toFixed(2)}s</td>
